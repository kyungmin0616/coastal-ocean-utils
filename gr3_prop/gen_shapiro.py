...
if __name__=="__main__":
    outfilename = './shapiro_ETOPO1.gr3'

    if os.path.exists(outfilename):
        os.remove(outfilename)

    write_shapiro(
        grd='../../../grid/05/ETOPO1/hgrid.utm',  # grid name (*.gr3 or *.npz, where *.npz is python format)
        #grd='/people/park075/Projects/ECGOM_GA/inputs/hgrid.utm',
        shapiro_max=0.5,
        threshold_slope=0.5,
        depths=[-99999, 20, 50],  # tweaks in shallow waters
        shapiro_vals1=[0.2, 0.2, 0.05],  # tweaks in shallow waters
        regions=[],  # tweaks in regions, the order matters
        shapiro_vals2=[],  # tweaks in regions, the order matters
        i_set_add_s=[],  # 0: set; 1: add
        fname=outfilename
    )
