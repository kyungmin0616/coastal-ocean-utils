...
shapiro_max=0.5
threshold_slope=0.5
s_depths=[-99999, 20,50]  # tweaks in shallow waters
shapiro_vals1=[0.2, 0.2,0.005]  # tweaks in shallow waters
...
if shapiro_max is not None:
    ...
    if s_depths is not None:
        ...
        fp = dp < s_depths[-1]
        shapiro[fp] = maximum(shapiro[fp], interp(array(s_depths), array(shapiro_vals1),dp[fp]))
