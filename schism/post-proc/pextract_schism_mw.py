...
for run in runs:
    ...
    gd.compute_all(); sys.stdout.flush()
    vd=read_schism_vgrid(run+'/vgrid.in')
    ...
    for svar in svars: 
       ...
       for istack in stacks:
           ...
           if irec%nproc==myrank: 
              read_schism_output(run,svar,bpfile,istack,ifs,nspool,hgrid=gd,vgrid=vd,fname=fname,zcor=1,fmt=itype,prj=prj)
