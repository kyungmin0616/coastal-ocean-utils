...
amp=[]; freq=[]; ts=loadz('{}/tide_fac_const.npz'.format(bdir))
for tname in tnames: 
...
for i,ibnd in enumerate(ibnds):
    ...
    nobn=gd.nobn[ibnd-1]; sind=gd.iobn[ibnd-1]; xi=gd.x[sind]; yi=gd.y[sind]; ap=[] #xi=mod(gd.x[sind]+360,360); yi=gd.y[sind]; ap=[]
    for m,tname in enumerate(tnames): 
        print('compute amp. and pha. for tide {} of boundary: {}'.format(tname,i+1))
        api=[]
        for n in arange(3):  
            if n==0: fname='{}/ocean_tide_extrapolated/{}.nc'.format(bdir,tname.lower()); an,pn='amplitude','phase'
            ...
            if any(clon>180 for clon in lon):
                print('Transforming FES2014 longitude to WGS')
                lon=(lon+180)%360-180; sindp=argsort(lon); lon=lon[sindp]
                amp0=amp0[:,sindp]; pha0=pha0[:,sindp]
