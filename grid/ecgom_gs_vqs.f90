...
!    ifort -O2 -mcmodel=medium -CB -o gen_44_vqs 44_vqs.f90 ./schism_geometry.f90
...
!     Read  vgrid.in
      ...
      !hsm=(/1, 2, 3, 4, 6, 8, 12, 18, 25, 33, 42, 52, 67, 80,&
      !     &100, 150, 230,350,650, 1000,&
      !     &1500, 2000, 3500, 5500, 7500, 9500/)
      !nv_vqs=(/2, 3, 4, 5, 7, 9, 13, 15, 17, 17, 20, 25, 30, 35,& 
      !    &44, 44, 44, 44, 44, 44,&
      !    &44, 44, 44, 44, 44, 44/)

      hsm=(/1, 8, 12, 18, 25, 33, 42, 52, 67, 80, 100,& !0~10
           &150, 230, 350, 650, 1000,& !11 ~ 15
           &1500, 2000, 3500, 5500, 7500, 8500/) ! 16 ~ 21
      nv_vqs=(/2, 6,  8, 11, 13, 15, 16, 18, 19, 20, 21,&
          &24, 26, 29, 32, 35,&
          &37, 39, 44, 48, 49, 49/)
...
!     Generate a master vgrid (z_mas)
      ...
      do m=1,m_vqs
          if (m<=2) then
             ...
             theta_f=6.2; theta_b=0.27
          end if
          print*, 'Theta f and b:', theta_f,theta_b
