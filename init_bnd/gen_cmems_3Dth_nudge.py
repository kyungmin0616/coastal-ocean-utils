...
#input
#------------------------------------------------------------------------------
StartT=datenum(2016,9,8); EndT=datenum(2016,11,1)
grd='../../../grid/05/ETOPO_11SM/grid.npz'
dir_hycom='/rcfs/projects/mhk_modeling/dataset/CMEMS/ECGOM/daily/'

#parameters to each files
iflags=[1,1,1,1,1,1]                    #if iflag=0: skip generating file 
dts=[1, 1, 1, 1, 1, 1]    #time steps for each file (day)
...
for n,[sname,svar,mvar,dt,iflag] in enumerate(zip(snames,svars,mvars,dts,iflags)):
    ...
    for m,fname in enumerate(fnames):
        ...
        ctime=array(C.variables['time'])/24+datenum(1950,1,1) #datestr2num(C.variables['time'].time_origin)
        sx=array(C.variables['longitude'][:]); sy=array(C.variables['latitude'][:]); sz=array(C.variables['depth'][:]); sz[0]=0; nz=len(sz)
        ...
        if not array_equal(sx,sx0)*array_equal(sy,sy0)*array_equal(sz,sz0):
            ...
            if ifix==0:
                ...
                cvs=array(C.variables['thetao'][0]); sindns=[]; sindps=[]
                for ii in arange(nz):
