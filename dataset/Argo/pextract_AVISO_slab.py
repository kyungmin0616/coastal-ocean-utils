...
CONFIG = {
    ...
    "start_date": (2022, 2, 1),
    "end_date": (2022, 4, 1),
    "output_base": Path("./npz/DUACS-global-adt-20220201-20220401"),
    "chunk_size": 30,  # number of files per chunk; tweak based on memory budget
    "merge_chunks": False,  # set True to build a single merged archive after chunking
    "coordinates": {
...
def merge_chunks_to_npz(
    ...
    sorted_entries = sorted(manifest, key=lambda item: item["chunk"])
    total_count = sum(int(entry.get("count", 0)) for entry in sorted_entries)
    if total_count == 0:
        return
    ...
    temp_dir = output_base.parent
    time_tmp = temp_dir / f"{output_base.name}_merge_time.tmp"
    adt_tmp = temp_dir / f"{output_base.name}_merge_adt.tmp"
    ugos_tmp = temp_dir / f"{output_base.name}_merge_ugos.tmp"
    vgos_tmp = temp_dir / f"{output_base.name}_merge_vgos.tmp"

    time_mm = np.memmap(time_tmp, dtype=np.float64, mode="w+", shape=(total_count,))
    adt_mm = np.memmap(adt_tmp, dtype=np.float32, mode="w+", shape=(total_count, *grid_shape))
    ugos_mm = np.memmap(ugos_tmp, dtype=np.float32, mode="w+", shape=(total_count, *grid_shape))
    vgos_mm = np.memmap(vgos_tmp, dtype=np.float32, mode="w+", shape=(total_count, *grid_shape))

    offset = 0
    for entry in sorted_entries:
        chunk_path = output_base.parent / entry["file"]
        npz = np.load(chunk_path)
        count = int(npz["time"].size)
        if count == 0:
            npz.close()
            continue
    ...
    merged_path = output_base.parent / f"{output_base.name}_merged.npz"
    np.savez_compressed(
        merged_path,
        time=np.asarray(time_mm),
        lon=longitude,
        lat=latitude,
        adt=np.asarray(adt_mm),
        ugos=np.asarray(ugos_mm),
        vgos=np.asarray(vgos_mm),
    )

    for mm in (time_mm, adt_mm, ugos_mm, vgos_mm):
        mm.flush()
        del mm

    for tmp in (time_tmp, adt_tmp, ugos_tmp, vgos_tmp):
        try:
            os.remove(tmp)
        except OSError:
            pass

    print(f"Saved merged archive to {merged_path}")
...
def main() -> None:
    ...
    if not MPI or RANK == 0:
        ...
        if config.get("merge_chunks"):
            merge_chunks_to_npz(chunk_manifest, output_base, longitude, latitude)
