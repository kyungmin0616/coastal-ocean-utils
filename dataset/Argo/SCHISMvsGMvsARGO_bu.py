...
CONFIG = {
    ...
    "schism": {
        ...
        "label": "SCHISM",
        "color": "b",
        "run_dir": "/storage/coda1/p-ed70/0/kpark350/Projects/NorthPacific/run/RUN01c",
        "variables": ["temp", "salt"],
    ...
    "global_model": {
        ...
        "label": "CMEMS",
        "color": "k",
        "data_dir": "/storage/coda1/p-ed70/0/kpark350/dataset/CMEMS/NorthPacific/",
        "file_suffix": ".nc",
        "variables": {
            "lon": "longitude",
            "lat": "latitude",
    ...
    "output": {
        "dir": "./argo_schism/each_profile/SCHISMvsCMEMSvsARGO/RUN01c",
    },
    "plot": {
        "linewidth": 1.5,
        "font_size": 7,
...
def extract_global_profile(cfg, stime, lon, lat):
    ...
    lon_raw = array(ds.variables[vars_["lon"]][:])
    if any(lon_raw > 180):
        lon_data = (lon_raw + 180) % 360 - 180
        lon_order = argsort(lon_data)
        lon_data = lon_data[lon_order]
    else:
        lon_data = lon_raw
        lon_order = None
    lat_data = array(ds.variables[vars_["lat"]][:])
    depth_data = array(ds.variables[vars_["depth"]][:])

    lon_grid, lat_grid = meshgrid(lon_data, lat_data)
    dist = (lat_grid - lat) ** 2 + (lon_grid - lon) ** 2
    llidx = unravel_index(dist.argmin(), dist.shape)

    temp_data = ds.variables[vars_["temp"]][:]
    salt_data = ds.variables[vars_["salt"]][:]
    fill_value = cfg.get("fill_value", -3e4)

    if lon_order is not None:
        temp_profile = array(temp_data[:, :, :, lon_order][:, :, llidx[0], :][:, :, llidx[1]])
        salt_profile = array(salt_data[:, :, :, lon_order][:, :, llidx[0], :][:, :, llidx[1]])
    else:
        temp_profile = array(temp_data[:, :, llidx[0], :][:, :, llidx[1]])
        salt_profile = array(salt_data[:, :, llidx[0], :][:, :, llidx[1]])

    temp_profile[temp_profile <= fill_value] = NaN
    salt_profile[salt_profile <= fill_value] = NaN

    temp_profile = squeeze(temp_profile)
    salt_profile = squeeze(salt_profile)

    if temp_profile.ndim > 1:
        temp_profile = temp_profile[0]
    if salt_profile.ndim > 1:
        salt_profile = salt_profile[0]
...
def extract_schism_profile(cfg, stime, lon, lat):
    ...
    C = read_schism_output(cfg["run_dir"], cvars, c_[lon, lat], istack, fmt=1)
...
def log_skip(reason, profile_id=None, stime=None, lon=None, lat=None):
    ...
    if lon is not None and lat is not None:
        parts.append(f"lon={lon:.2f},lat={lat:.2f}")
...
def main():
    ...
    if len(active_models) == 0:
        print("No models selected for comparison. Enable at least one model in CONFIG.")
        return

    ensure_output_dir(CONFIG["output"]["dir"])
    dir_argo = CONFIG["argo"]["dir"]

    for idx, fname in enumerate(argo_files):
        print(f"Working on {idx + 1}/{len(argo_files)}")
        ...
        for nn, stime in enumerate(stimes):
            ...
            if stime < start_t or stime > end_t:
                log_skip("outside target window", argo_ids[idx], stime, lon_pt, lat_pt)
                continue
            if not point_in_region(region, lon_pt, lat_pt):
                log_skip("outside configured region", argo_ids[idx], stime, lon_pt, lat_pt)
                continue
            ...
            if valid.sum() < 4:
                log_skip("insufficient depth samples", argo_ids[idx], stime, lon_pt, lat_pt)
                continue
            obs_depth_clean = obs_depth[valid]
            obs_temp_clean = obs_temp[valid]
            obs_salt_clean = obs_salt[valid]
            if len(obs_depth_clean) == 0:
                log_skip("no valid observation depths", argo_ids[idx], stime, lon_pt, lat_pt)
                continue
            ...
            if gm_cfg is not None:
                ...
                if gm_profile is not None:
                    ...
                    if obs_subset is not None:
                        ...
                        log_skip("global model depth range mismatch", argo_ids[idx], stime, lon_pt, lat_pt)
                else:
                    log_skip("global model profile missing", argo_ids[idx], stime, lon_pt, lat_pt)
            if sch_cfg is not None:
                ...
                if sch_profile is not None:
                    ...
                    if obs_subset is not None:
                        ...
                        log_skip("SCHISM depth range mismatch", argo_ids[idx], stime, lon_pt, lat_pt)
                else:
                    log_skip("SCHISM profile missing", argo_ids[idx], stime, lon_pt, lat_pt)

            if len(models) == 0:
                log_skip("no models available for comparison", argo_ids[idx], stime, lon_pt, lat_pt)
                continue
